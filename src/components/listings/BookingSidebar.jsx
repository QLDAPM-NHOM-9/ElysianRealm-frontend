...
const OrderSummary = ({ title, subTitle, imageUrl, showReviews = true, rating, reviewCount }) => (
          ...
          {reviewCount && reviewCount > 0 ? (
            <>
              <span className="flex items-center gap-1 text-sm font-medium text-brand-primary">
                {rating.toFixed(1)} <FiStar className="w-3 h-3 fill-current" />
              </span>
              <span className="text-text-secondary">
                {`${rating >= 4.5 ? 'Excellent' : rating >= 4.0 ? 'Very Good' : rating >= 3.5 ? 'Good' : 'Average'} (${reviewCount} reviews)`}
              </span>
            </>
          ) : (
            <span className="text-text-secondary">New Tour</span>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <aside className="w-full lg:w-96 bg-bg-primary p-6 rounded-lg shadow-sm sticky top-28">
      ...
      <OrderSummary
        ...
        reviewCount={orderDetails.reviewCount}
      />
